...
public class IntegrationConfiguration {
    ...
    public ChunkMessageChannelItemWriter chunkWriter() {
        ...
        chunkWriter.setMessagingOperations(messagingTemplate());
        chunkWriter.setReplyChannel(inboundReplies()); //listen to replies
    ...
    public MessagingTemplate messagingTemplate() {
        MessagingTemplate messagingTemplate = new MessagingTemplate(outboundRequests()); //configuring to write to outboundrequests channel
        messagingTemplate.setReceiveTimeout(60000000l);
        return messagingTemplate;
    }

    @Bean
    @ServiceActivator(inputChannel = "outboundRequests")//using rabbitmq for persisting communication
    public AmqpOutboundEndpoint amqpOutboundEndpoint(AmqpTemplate template) {
        ...
        endpoint.setRoutingKey(CHUNKING_REQUESTS); //listen for work in this queue
        return endpoint;
    }
    ...
    @Bean
    @Profile("slave")
    public AmqpInboundChannelAdapter inboundRequestAdapter(SimpleMessageListenerContainer listenerContainer) {
    ...
    public ChunkProcessorChunkHandler chunkProcessorChunkHandler(ItemProcessor<Customer, CustomerWrite> itemProcessor, ItemWriter<CustomerWrite> itemWriter) throws Exception {
